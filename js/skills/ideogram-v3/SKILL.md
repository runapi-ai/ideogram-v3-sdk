...
- For a one-off artifact or result, use the registered `ideogram-v3` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi ideogram-v3 <operation> --help
curl --fail --location https://runapi.ai/docs/api/ideogram-v3/<operation>.md --output contract.md
```
...
```shell
runapi ideogram-v3 <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service ideogram-v3 --action <operation> > result.json
```
...
Use this route only for application or production-code integration. Open the current RunAPI SDK reference below, select the package for the target language and `Ideogram V3`, and confirm its install command, client methods, request types, response types, and error classes before coding. Build the request from the same discovered product contract and apply the same deliverable verification and stop rules. Do not invoke `runapi` as a subprocess from production code.

## References

- Model overview, pricing, and rate limits: https://runapi.ai/models/ideogram-v3.md
- Provider overview: https://runapi.ai/providers/ideogram.md
- Full model catalog: https://runapi.ai/models.md
- SDK integration: https://github.com/runapi-ai/ideogram-v3-sdk

## Variants
- `ideogram-v3-character`: https://runapi.ai/models/ideogram-v3/character.md
- `ideogram-v3-character-edit`: https://runapi.ai/models/ideogram-v3/character-edit.md
- `ideogram-v3-character-remix`: https://runapi.ai/models/ideogram-v3/character-remix.md
- `ideogram-v3-edit`: https://runapi.ai/models/ideogram-v3/edit.md
- `ideogram-v3-reframe`: https://runapi.ai/models/ideogram-v3/reframe.md
- `ideogram-v3-remix`: https://runapi.ai/models/ideogram-v3/remix.md
- `ideogram-v3-text-to-image`: https://runapi.ai/models/ideogram-v3/text-to-image.md
